...
import (
	...
	"xback/userop_srv/global"
	"xback/userop_srv/model"
	"xback/userop_srv/proto"
)
...
func (*UserOpServer) DeleteAddress(ctx context.Context, req *proto.AddressRequest) (*emptypb.Empty, error) {
	if result := global.DB.Where("id=?", req.Id).Delete(&model.Address{}); result.RowsAffected == 0 {
...
func (*UserOpServer) UpdateAddress(ctx context.Context, req *proto.AddressRequest) (*emptypb.Empty, error) {
	...
	if result := global.DB.Where("id=? ", req.Id).First(&address); result.RowsAffected == 0 {
