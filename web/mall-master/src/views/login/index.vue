    ...
    <el-card class="login-form-layout">
      <el-form autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left">
        ...
        <h2 class="login-title color-main">
          <h3>校园易购二手平台</h3>-后台管理系统-
        </h2>
        <el-form-item prop="mobile">
          <el-input name="mobile" type="text" v-model="loginForm.mobile" autoComplete="on" placeholder="请输入用户名">
            <span slot="prefix">
        ...
        <el-form-item style="margin-bottom: 60px;text-align: center">
          <el-input name="captcha" type="text" v-model="loginForm.captcha" autoComplete="on" placeholder="请输入验证码">
            <img slot="suffix" :src="captcha.picPath" alt="" v-if="captcha.picPath" @click="changeCaptcha"
              class="captcha-img">
        ...
        this.$refs.loginForm.validate(valid => {
          if (valid) {
            ...
            login(this.loginForm).then(response => {
              ...
              _this.loading = false;
              _this.errorMessage =  '登录失败，请稍后重试'; // 捕获并设置错误信息
            });
          }
        });
      },
