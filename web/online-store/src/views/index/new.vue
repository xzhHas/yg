<template>
    <div>
        ...
        <div class="newopro-r fr">
            <h2 class="index-tt">
                <em class="ft18 c000">最新上架物品</em>
                <router-link to="/app/home/list/more"><a  class="fr c666">更多&gt;&gt;</a></router-link>
            </h2>
            <ul class="newgoods_fastbuy">
                <li class="prolist-cent clearfix have_num" v-for="item in newopro">
                    <div class="prolist-l fl">
                    <router-link :to="'/app/home/productDetail/'+item.id" > <a  :title="item.name" class="imgBox">
                    <img :src="item.front_image" style="height: 158px;width: 158px;" class="zom" :alt="item.name">
                    </a></router-link>
                    </div>
                    <div class="prolist-r fl">
                        <h3 class="ft14 c333 bold">
                        <router-link :to="'/app/home/list/'+item.id"  :title="item.name" >{{item.name}}</router-link>
                        </h3>
                        <p><em class="c333"></em>简介：{{item.goods_brief}}</p><div>
...
<script>
  import { getNewHomeGoods } from '../../api/api';
export default{
    data(){
        return {
            newopro:{}
        ...
        getOpro(){
            ...
            .then((response)=> {
               //跳转到首页页response.body面
                this.newopro = response.data.data
            })
