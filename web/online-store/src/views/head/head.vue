            ...
            <div class="bd_bar_bd cle">
                <ul class="welcome">
                    <li id="ECS_MEMBERZONE" v-if="userInfo.name">
                        <router-link :to="'/app/home/member/userinfo'">登录用户名：{{ userInfo.name }}</router-link>
                        &nbsp;[
                ...
                <ul id="userinfo-bar">
                    <li class="more-menu" @mouseover="isShowVip = true" @mouseout="isShowVip = false"> <a>【个人功能中心】</a><i
                        ...
                        <div class="more-bd" :class="{ show: isShowVip }">
                            <div class="list">
                                ...
                                <router-link :to="'/app/home/member/receive'">收货地址</router-link>
                            </div>
                        </div>
                    </li>
                </ul>
        ...
        <div class="hd_main cle">
            <div class="logo">
                <router-link to="/app/home/index" class="lizi_logo">
                    <img src="../../static/images/head/logo.gif" alt="校园易购二手平台">
                </router-link>
            ...
            <div class="head_search_hot">
                <span>热搜榜：</span>
                <router-link v-for="item in hotSearch.slice(0, 5)" :to="'/app/home/productDetail/' + item.id"
                    :key="item.id">
                    {{ item.name }}
                </router-link>
            </div>
            <div class="intro">
                <ul>
                    <!-- 这里的href后的链接都需要修改为 我的个人博客，这里暂时不做改变 -->
            ...
            <div class="hd_nav_bd cle">
                        ...
                        <div class="main_cata" id="J_mainCata" v-show="showAllmenu">
                            <ul>
                                <li class="first" v-for="(item, index) in allMenuLabel"
                                    ...
                                    <h3
                                        style="background:url(../../static/images/head/1449088788518670880.png) 20px center no-repeat;">
                                    ...
                                    <div class="J_subCata" id="J_subCata" v-show="showChildrenMenu === index"
                                        ...
                                        <div class="J_subView">
                                            <div v-for="list in item.sub_category">
                                                <dl>
                                                    <dt>
                                                        <router-link :to="'/app/home/list/' + list.id">{{ list.name
                                                            }}</router-link>
                                                    </dt>

                                                    <dd>
                                                        <router-link v-for="childrenList in list.sub_category"
                                                            :key="childrenList.id"
                                                            :to="'/app/home/list/' + childrenList.id">{{
                ...
                <div class="hd_cart" id="ECS_CARTINFO" @mouseover="overShopCar" @mouseout="outShopCar">
                    <router-link class="tit" :to="'/app/shoppingcart/cart'">
                        ...
                        <em class="num" id="hd_cartnum" style="visibility: visible;">{{ goods_list.length
                            }}</em></router-link>
                    <div class="list" v-show="showShopCar">
                        <div class="data">
                            <dl v-for="(item, index) in goods_list.goods_list">
                                <dt><router-link :to="'/app/home/productDetail/' + item.goods_id" target=_blank><img
                                            :src="item.good_image"></router-link></dt>
                                <dd>
                                    <h4><router-link :to="'/app/home/productDetail/' + item.goods_id" target=_blank>{{
                                            item.good_name
                                            }}</router-link></h4>
                                    <p><span class="red">{{ item.goods_price }}</span>&nbsp;<i>X</i>&nbsp;{{ item.nums
                                        }}</p>
                                    <a title="删除" class="iconfont del" @click="deleteGoods(index, item.goods_id)">×</a>
                                </dd>
                            </dl>
                        </div>
...
import { getHotSearch, getCategory, deleteShopCart, searchGo, getHot } from '../../api/api'
export default {
    data() {
        return {
            hotSearch: [],//热词
        ...
        getHotSearch() {//获取热搜
            getHot()
                .then((response) => {
                    ...
                    console.log('热搜来啊里', this.hotSearch);
                })
                .catch(function (error) {
                    console.log(error);
                });
