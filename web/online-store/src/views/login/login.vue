        ...
        <div class="fr form-box">
          ...
          <form id="jsLoginForm" autocomplete="off">
            ...
            <div class="form-group marb20">
              <label>用&nbsp;户&nbsp;名</label>
              <input name="account_l" id="account_l" type="text" v-model="userName" @focus="errorUnshow"
                placeholder="手机号/账号">
            </div>
            <p class="error-text" v-show="error.mobile">{{ error.mobile }}</p>

            <div class="form-group marb8">
              <label>密&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;码</label>
              <input name="password_l" id="password_l" type="password" v-model="parseWord" @focus="errorUnshow"
                placeholder="请输入您的密码">
            </div>
            <p class="error-text" v-show="error.mobile">{{ error.password }}</p>

            <div class="form-group form-groups marb8">
              <label>验&nbsp;证&nbsp;码</label>
              <input name="captcha" id="captcha" type="captcha" v-model="captchacode" @focus="errorUnshow"
                placeholder="请输入验证码">
              <img :src="captcha" alt="" @click="changeCaptcha" class="captcha-img">
            </div>
            <p class="error-text" v-show="error.captcha">{{ error.captcha }}</p>
  ...
  methods: {
    ...
    login() {
      ...
      login({
        ...
        //本地存储用户信息
        cookie.setCookie('name', response.data.nick_name, 7);
        ...
        .catch(function (error) {
          that.error.mobile = error.error.mobile ? error.error.mobile : '';
          that.error.password = error.error.password ? error.error.password : '';
          that.error.captcha = error.error.captcha ? error.error.captcha : '';
